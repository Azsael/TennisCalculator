        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in lines.WithCancellation(cancellationToken))
        {
            ...
            if (trimmedLine.StartsWith("Match:", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    throw new TennisDataSourceException($"Invalid match header format at line {lineNumber}: Match ID cannot be empty", "tournament file", lineNumber);
                }
            ...
            if (trimmedLine.Contains(" vs ", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    throw new TennisDataSourceException($"Player names found at line {lineNumber} without preceding match header", "tournament file", lineNumber);
                }
                
                var parts = trimmedLine.Split(" vs ", StringSplitOptions.RemoveEmptyEntries);
                if (parts.Length != 2)
                {
                    throw new TennisDataSourceException($"Invalid player format at line {lineNumber}: Expected '<Player1> vs <Player2>'", "tournament file", lineNumber);
                }
                ...
                {
                    throw new TennisDataSourceException($"Invalid player names at line {lineNumber}: Player names cannot be empty", "tournament file", lineNumber);
                }
                
                if (p1.Equals(p2, StringComparison.OrdinalIgnoreCase))
                {
                    throw new TennisDataSourceException($"Invalid player names at line {lineNumber}: Players cannot have the same name", "tournament file", lineNumber);
                }
            ...
            else if (currentMatchId is not null)
            {
                ...
                {
                    throw new TennisDataSourceException($"Expected player names after match header at line {lineNumber}", "tournament file", lineNumber);
                }
            }
        }
        ...
        {
            throw new TennisDataSourceException("Incomplete match data: Missing player names for match", "tournament file");
        }
    }
}